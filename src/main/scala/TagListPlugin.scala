...
object TagListPlugin extends AutoPlugin {
  ...
  object autoImport {
    ...
    lazy val tagListTags = SettingKey[Set[Tag]]("tag-list-tags", "Detailed tags with a log level per tag word")
  ...
  override lazy val projectSettings = Seq(
    ...
    tagListTags := tagListWords.value.map(word => Tag(word, Warn)),
    tagListSkipChars := Set('/', ':'),
    sources in tagList := (sources in Compile).value,
    tagList := {
      val srcs: Seq[File] = sources.in(tagList).value
      val tagWords = tagListTags.value
      val s = streams.value
      val skip = tagListSkipChars.value


      val result = TagListGenerator.generateTagList(srcs, tagWords, skip, Codec.UTF8)

      val count = result.map(_._2.size).sum
      if (count > 0) {
